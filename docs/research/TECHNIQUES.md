...
| A10c | Plasma Lava Lamp | A10 + C11: per-cell 24-bit color driven by same plasma float that drives char selection (high→white/yellow, mid→orange/red, low→deep violet). Both char and color shift simultaneously per frame on same field at different scales. Looks like a lava lamp. No new fill code — wire plasma float grid into C11. | 4 | `idea` |
