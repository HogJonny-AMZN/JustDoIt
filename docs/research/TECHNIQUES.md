...
| A_SLIME1 | Slime Mold Time-Lapse Animation | Physarum polycephalum agent simulation captured at increasing step counts — each frame shows network formation at a more mature stage: random walks → faint traces → preferred paths → stable vein network. Single-pass snapshot approach (runs sim once per glyph, captures at specified steps). With loop=True, produces formation+dissolution cycle. BIO palette + C12 bloom. | 4 | `done 2026-04-26` |
...
| X_TURING_WARP | Turing Morphogenesis-Modulated Sine Warp | N09 Turing FHN activator float grid drives per-row sine_warp amplitude. Rows with heavy high-activator clusters warp dramatically; rows at inhibitor troughs are nearly still. Both color (C11 BIO_PALETTE) and amplitude topology are derived from the same static Turing float grid — letters appear to deform in the geometry of their own biological skin. Only the sine phase_offset sweeps per frame. New `phase_offset` param added to `sine_warp()` (backward-compatible). N09 (Turing float) × S01 (per-row amplitude) × C11 BIO_PALETTE × C12 green bloom. 72 frames @ 12fps (36×2 loop). Scores: tension=5 emerge=5 distinct=5 wow=5 → 20/20. | 5 | `done 2026-04-19` |
| X_FLAME_ISO_BLOOM | Flame + Isometric + Bloom — Three-Axis Flagship | A08 Doom-fire fill + S03 isometric extrusion + C12 bloom. Front face: stochastic flame chars (re-seeded per frame) colored with C11 fire palette. Depth face (▓▒░·): ember float values (0.05-0.25) → orange-red ember coloring from fire palette. C12 bloom: fire-orange light bleeds into surrounding space. Three maximally different forces: thermal chaos × rigid geometry × light emission. The project's long-planned 3D-fire flagship. `flame_iso_bloom()` in presets.py. 72 frames @ 12fps (36×2 loop). Scores: tension=5 emerge=5 distinct=5 wow=5 → 20/20. | 5 | `done 2026-04-20` |
| X_NOISE_WARP | Perlin Noise Phase-Map Sine Warp | F02 Perlin noise float grid (static, seeded) drives per-row phase offset in ``sine_warp()`` (new ``phase_map`` param). Each row reaches its sine peak displacement at a different moment determined by its mean noise value. Global ``phase_offset`` sweeps 0→2π per frame to animate the warp. Key insight: amplitude_map (X_PLASMA_WARP, X_TURING_WARP) controls HOW FAR a row moves; phase_map controls WHEN it peaks — categorically different visual character: rippled-glass / crinkled-cellophane distortion. New infrastructure: ``noise_float_grid()`` companion + ``phase_map`` param on ``sine_warp()``. ``noise_warp()`` preset in presets.py. C11 spectral color from same noise float; C12 cyan bloom. 72 frames @ 12fps (36×2 loop). Scores: tension=4 emerge=4 distinct=5 wow=3 → 16/20 (wow revised to 3 after CB6: moderate phase spread makes effect pleasingly subtle but less visually dramatic than 4). | 5 | `done 2026-04-21` |
| X_PLASMA_NOISE_WARP | Plasma Amplitude × Noise Phase Sine Warp | Maximum FILL→SPATIAL coupling: two independent float fields on two independent params of sine_warp() simultaneously. Plasma (dynamic, per-frame-varying) → amplitude_map: rows swing different distances, topology rotates each frame. Noise (static, crystalline) → phase_map: rows peak at fixed timing offsets. Combined: each row differs in BOTH magnitude AND timing — neither parent alone produces this. Letters appear embedded in a flowing medium with anisotropic internal dynamics. C11 spectral color from plasma float (dynamic); C12 cyan bloom. `plasma_noise_warp()` in presets.py. 72 frames @ 12fps (36×2 loop). Scores: tension=5 emerge=5 distinct=5 wow=4 → 19/20. | 5 | `done 2026-04-22` |
| X_ISO_NEON | Isometric Neon Glitch | S03 isometric extrusion × A03n neon glitch × C12 bloom. Front face: stable bright neon foreground color, letters read clearly. Depth face (▓▒░·): each char independently flickers between dim (60%), flicker-fringe (25%), or spark (15%) states — stochastic electrical discharge on the extrusion face. C12 bloom bleeds neon hue into surrounding space. Key tension: fixed front-face geometry × stochastic depth face. Each frame unique, letterforms permanently legible. iso_neon_glitch() in presets.py. 72 frames @ 12fps (36×2 loop). Scores: tension=4 emerge=4 distinct=4 wow=4 → 16/20. CB6 verdict: ✅ Meets bar — depth face flicker clearly reads as electrical discharge, front face stable neon is clean, bloom halo is crisp. The front/depth tension is visually legible and distinct from all other techniques in the gallery. | 4 | `done 2026-04-26` |
| X_LIVING_COLOR | Living Color — GoL Age-Heat | A06 Conway GoL (living_fill) × C11 AGE_PALETTE coloring. Each alive cell accumulates a consecutive-alive age counter; age/max_age → [0.0, 1.0] → AGE_PALETTE (blue=newborn → cyan → green → amber → red=ancient). Dead interior cells rendered as dim dot chars with 0.0 float (cold-blue floor). What emerges: metabolic map of CA stability — transient cells flicker blue while rare stable still-lifes and blinkers burn red. The stable-structure distribution in a 7-row glyph Conway grid is bimodal: ~12 red stable cells + ~154 blue transients. C12 cyan bloom. `living_color()` in presets.py. AGE_PALETTE added to color.py and PALETTE_REGISTRY. 72 frames @ 10fps (144 with loop). Scores: tension=4 emerge=4 distinct=4 wow=3 → 15/20. CB6 verdict: ⚠️ Partial — CA converges quickly in small masks leaving a static bimodal color pattern; the stability structure is accurate and legible but less dynamically interesting than predicted. | 4 | `done 2026-04-24` |
